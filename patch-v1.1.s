...
init_device:
	...
	ld    (P3CR),0A0H				;	Load $20 into P3CR register		(1010 0000)				-	Set: WAIT: no-wait; /RD as fixed pin; P33 to Open-Drain; P30, P31 as input ports; P32, P33 as output ports
	ld    (P4CR),00FH				;	Load $0F into P4CR register		(0000 1111)				-	Set: Port 4 as address bus (4 MSB of address bus: A16-A19)  (0 value denotes output port; 1 value denotes addresss bus)
...
copy_byte_step:
	ld    d,(hl)					;	Copy 1-byte data from source address to D register
	ld    (ix),d					;	Copy 1-byte data from D register to destination address
data_polling:
	nop
	nop
	...
	jp    nz,copy_byte_step			;	... if not, jump back to copy next word
