...
EXTERNAL_COPY_PROCESS = 02100H
EXTERNAL_ROM_END = 0FEBFH
INTERNAL_RAM_START = 0FEC0H
INTERNAL_RAM_END = 0FFBFH

EXTERNAL_STACK_START = 03000H		;	Start address of predefined values to be taken in RETI instruction
EXTERNAL_STACK_POINTER = 03004H		;	Carefully chosen to allow Interrupt acknowledge cycles to write:  $3003: PCH,  $3002: PCL,  $3001: A  and  $3000: F
...
; MESSAGE: 'NMK215 Test-StackPt Code by Recreativos Piscis(C)2023 (a)Recre_Piscis '
	db    4EH
	db    4DH
	db    4BH
	db    32H
...
decrement_sp:
	...
	halt							;	Stop and wait for reset or interrupt
...
disable_watchdog:
	ld    (WDMOD),74H				;	Load $74 into WDMOD register	(0111 0100)				-	Set: Watchdog disable; Detecting period and warm-up values (not used); STOP mode for HALT; Drive 0 on STOP mode
...
copy_sp_value:
	ld    ix,0000H
	ld    de,sp						;	Copy 2-byte data value of SP to DE registers
	ld    (ix),d					;	Copy first byte from D register to destination address
data_polling_spd:
	nop
	nop
	...
	cp	  a,d						;	Check if the byte read in the polling is the same as the original byte written...
	jp    nz,data_polling_spd		;	... if not, jump back to keep polling until write cycle has ended	
	
	inc   ix						;	increment destination address
	ld    (ix),e					;	Copy second byte from E register to destination address
data_polling_spe:
	nop
	nop
	ld	  a,(ix)					;	Perform a "Data-Polling" by reading the content on the same address where the byte was written and read back to A register, to check if the write cycle has ended (see 28C64B datasheet)
	cp	  a,e						;	Check if the byte read in the polling is the same as the original byte written...
	jp    nz,data_polling_spe		;	... if not, jump back to keep polling until write cycle has ended	
	

copy_wdmod_value:
	inc   ix						;	increment destination address
	ld    d,(WDMOD)					;	Copy 1-byte data value of WDMOD to D register
	ld    (ix),d					;	Copy byte from D register to destination address
data_polling_wdmodd:
	nop
	nop
	ld	  a,(ix)					;	Perform a "Data-Polling" by reading the content on the same address where the byte was written and read back to A register, to check if the write cycle has ended (see 28C64B datasheet)
	cp	  a,d						;	Check if the byte read in the polling is the same as the original byte written...
	jp    nz,data_polling_wdmodd	;	... if not, jump back to keep polling until write cycle has ended	
